...
function buildCmdkIndex() {
  ...
  if (typeof FLOWS !== "undefined") {
    Object.entries(FLOWS).forEach(([id, f]) => cmdkIndex.push({ type:"flow", id:id, name:(f.titulo || f.nome || id), sis:f.sis||"", sin:[] }));
  }
}
...
function injectFooterMeta(type, id) {
  ...
  meta.innerHTML = `
    ...
    <span class="review-meta-item">✍️ por vovôminic</span>
    <span class="review-meta-item">⚠ Material educacional · verifique sempre · responsabilidade do médico assistente</span>
  `;
  ...
  pf.innerHTML = `
    <strong>Vovôminic Doutô — Guia Clínico</strong> · por vovôminic<br>
    Impresso em ${todayBR} · felipertoledo.github.io/plantao<br>
    <strong>Aviso:</strong> material educacional. Informações podem conter erros — verifique nas diretrizes-fonte. A responsabilidade da prescrição é exclusivamente do médico assistente. O autor não se responsabiliza por decisões clínicas decorrentes do uso destas informações.
  `;
  view.appendChild(pf);
}
