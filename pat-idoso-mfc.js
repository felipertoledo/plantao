...
A) FERRAMENTAS DE AVALIAÇÃO:
  ...
  • Furosemida em edema sem ICC documentada
...
C) CASOS COMUNS:
  ...
  → Substituir por metformina. Meta HbA1c 7,5-8%.
  
  Idoso 85a com AAS prevenção primária, HAS controlada:
  → Suspender AAS (Beers).
  
  Idoso 76a com cascata AINE→HAS→losartana+tiazida→tontura→flunarizina→parkinsonismo→biperideno:
  → Suspender flunarizina + biperideno (cascata). Reavaliar AINE.
...
D) MANEJO FARMACOLÓGICO:
  ...
  • Se SINTOMAS GRAVES com risco (agressão, agitação intensa):
    ...
    - Quetiapina baixa dose em demência por corpos de Lewy (não usar haldol — piora muito)
...
D) MANEJO:
  ...
  • Olanzapina, ziprasidona: alternativas
...
padrao_ouro:"NOF (National Osteoporosis Foundation), AACE, SBR (Sociedade Brasileira de Reumatologia), IOF (International Osteoporosis Foundation). FRAX brasileiro disponível online. Bisfosfonatos têm a maior evidência de redução de fraturas (vertebral 40-70%, quadril 20-40%). Denosumabe, anabólicos e medicações novas são padrão em casos selecionados.",
